...
@section('title', __('Buat Kontrak Elektronik'))
...
<div class="container-fluid">
    <div class="page-inner">
        <div class="d-flex align-items-left align-items-md-center flex-column flex-md-row pt-2 pb-4 gap-2">
            <div>
                <h4 class="fw-bold mb-1">{{ __('Buat Kontrak') }}</h4>
                <small class="text-muted">{{ __('Pilih metode tanda tangan elektronik atau manual sesuai proses HR.') }}</small>
            </div>
        ...
        @if($templates->isEmpty())
            <div class="alert alert-warning">
                {{ __('Belum ada template aktif. Buat template kontrak terlebih dahulu sebelum generate kontrak karyawan.') }}
            </div>
        @endif
        ...
        <form action="{{ route('electronic-contracts.store') }}" method="POST">
            ...
            <div class="row g-3">
                <div class="col-lg-8">
                            ...
                            <div class="row g-3">
                                <div class="col-12">
                                    ...
                                    <select
                                        ...
                                        data-placeholder="{{ __('Ketik minimal 2 karakter nama atau NIK') }}">
                                        @if($selectedEmployee)
                                ...
                                <div class="col-md-4">
                                    <label class="form-label">{{ __('No Kontrak') }}</label>
                                ...
                                <div class="col-md-4">
                                    <label class="form-label">{{ __('Kode Kontrak') }}</label>
                                ...
                                <div class="col-md-4">
                                    <label class="form-label">{{ __('No PKWT') }}</label>
                                ...
                                <div class="col-md-4">
                                    <label class="form-label">{{ __('Durasi Kontrak') }}</label>
                                    <input type="text" name="contract_duration" class="form-control @error('contract_duration') is-invalid @enderror" value="{{ old('contract_duration') }}" placeholder="{{ __('Contoh: 12 bulan') }}">
                                    @error('contract_duration')<div class="invalid-feedback">{{ $message }}</div>@enderror
                                </div>
                                <div class="col-md-4">
                                    <label class="form-label">{{ __('Tanggal Mulai Kontrak') }}</label>
                                ...
                                <div class="col-md-4">
                                    <label class="form-label">{{ __('Tanggal Berakhir Kontrak') }}</label>
                                ...
                                <div class="col-md-6">
                                    <label class="form-label">{{ __('Gaji / Upah Terbaru') }}</label>
                                ...
                                <div class="col-12 js-addendum-fields">
                                    <div class="contract-form-note p-3">
                                        <div class="fw-semibold mb-1">{{ __('Data Adendum') }}</div>
                                ...
                                <div class="col-md-4 js-addendum-fields">
                                    <label class="form-label">{{ __('Durasi Perpanjangan Pertama') }}</label>
                                    <input type="text" name="first_extension_duration" class="form-control @error('first_extension_duration') is-invalid @enderror" value="{{ old('first_extension_duration') }}" placeholder="{{ __('Contoh: 6 bulan') }}">
                                    @error('first_extension_duration')<div class="invalid-feedback">{{ $message }}</div>@enderror
                                </div>
                                <div class="col-md-4 js-addendum-fields">
                                    <label class="form-label">{{ __('Tanggal Perpanjangan Pertama Berakhir') }}</label>
                                ...
                                <div class="col-md-4 js-addendum-fields">
                                    <label class="form-label">{{ __('Klausul Adendum 1') }}</label>
                                    ...
                                    <small class="text-muted">{{ __('Jika adendum ke-2 dan seterusnya, sistem otomatis memakai Klausul 2.') }}</small>
                    ...
                    <div class="d-flex gap-2 mt-3">
                        <button class="btn btn-primary" type="submit" {{ $templates->isEmpty() ? 'disabled' : '' }}>
                            <i class="fas fa-file-signature me-1"></i> {{ __('Buat Kontrak') }}
                        </button>
                        <a href="{{ route('electronic-contracts.index') }}" class="btn btn-outline-secondary">{{ __('Batal') }}</a>
                    </div>
                </div>
                ...
                <div class="col-lg-4">
                    <div class="card border-0 shadow-sm">
                        <div class="card-body">
                            <h5 class="mb-2">{{ __('Catatan Alur') }}</h5>
                            <ul class="small text-muted ps-3 mb-0">
                                <li>{{ __('Metode elektronik membuat kontrak tampil di menu Kontrak Elektronik karyawan aktif.') }}</li>
                                <li>{{ __('Metode manual dipakai untuk kontrak cetak lalu hasil scan diunggah kembali oleh HR.') }}</li>
                                <li>{{ __('PDF final elektronik baru disimpan setelah karyawan menandatangani.') }}</li>
                                <li>{{ __('Audit log mencatat pembuat kontrak, pembuka dokumen, dan tanda tangan.') }}</li>
                            </ul>
                        </div>
                    </div>
                </div>

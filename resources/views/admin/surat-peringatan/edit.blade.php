                ...
                <form action="{{ route('surat-peringatan.update', $suratPeringatan->id) }}" method="POST">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-12 mb-3">
                            <label class="form-label">{{ __('Pilih level surat peringatan') }}</label>
                            <select name="level_sp" class="form-control form-select" required>
                                <option value="SP1" {{ old('level_sp', $suratPeringatan->level_sp) == 'SP1' ? 'selected' : '' }}>{{ __('Surat Peringatan 1') }}</option>
                                <option value="SP2" {{ old('level_sp', $suratPeringatan->level_sp) == 'SP2' ? 'selected' : '' }}>{{ __('Surat Peringatan 2') }}</option>
                                <option value="SP3" {{ old('level_sp', $suratPeringatan->level_sp) == 'SP3' ? 'selected' : '' }}>{{ __('Surat Peringatan 3') }}</option>
                            </select>
                        </div>

...
@section('title', __('Edit Master Jadwal Kerja'))
...
<div class="container-fluid">
    <div class="page-inner">
        <div class="d-flex justify-content-between align-items-center pt-2 pb-4">
            <div>
                <h4 class="fw-bold mb-1">{{ __('Edit Master Jadwal Kerja') }}</h4>
                <small class="text-muted">{{ __('Perbarui pola kerja yang dipakai sebagai acuan penjadwalan.') }}</small>
            </div>
            <a href="{{ route('work-patterns.index') }}" class="btn btn-light">{{ __('Kembali') }}</a>
        </div>
        ...
        <div class="card">
            <div class="card-body">
                <form action="{{ route('work-patterns.update', $workPattern->id) }}" method="POST" class="row g-3">
                    ...
                    @method('PUT')
                    <div class="col-12">
                        <label class="form-label">{{ __('Preset Cepat') }}</label>
                        ...
                        <small class="text-muted d-block mt-2">{{ __('Preset ini hanya mengisi pola kerja. Jam masuk, jam pulang, dan jadwal istirahat tetap bisa Anda atur sendiri.') }}</small>
                    </div>
                    <div class="col-md-4">
                        <label class="form-label">{{ __('Kode') }}</label>
                        <input type="text" id="patternCode" name="code" class="form-control @error('code') is-invalid @enderror" value="{{ old('code', $workPattern->code) }}">
                        @error('code')<div class="invalid-feedback">{{ $message }}</div>@enderror
                    </div>
                    <div class="col-md-8">
                        <label class="form-label">{{ __('Nama Pola Kerja') }}</label>
                        <input type="text" id="patternName" name="name" class="form-control @error('name') is-invalid @enderror" value="{{ old('name', $workPattern->name) }}">
                        @error('name')<div class="invalid-feedback">{{ $message }}</div>@enderror
                    </div>
                    <div class="col-md-4">
                        <label class="form-label">{{ __('Basis Pola Kerja') }}</label>
                        <select name="pattern_basis" id="patternBasis" class="form-select @error('pattern_basis') is-invalid @enderror">
                            @foreach($basisOptions as $value => $label)
                                <option value="{{ $value }}" {{ old('pattern_basis', $workPattern->pattern_basis ?: \App\Models\WorkPattern::BASIS_CYCLE) === $value ? 'selected' : '' }}>{{ $label }}</option>
                            @endforeach
                        </select>
                        @error('pattern_basis')<div class="invalid-feedback">{{ $message }}</div>@enderror
                    </div>
                    <div class="col-md-8 js-weekly-section">
                        <label class="form-label">{{ __('Hari Kerja Mingguan') }}</label>
                            ...
                            @foreach($weekdayOptions as $value => $label)
                                <div class="form-check">
                                    <input
                                        ...
                                        {{ in_array($value, old('weekly_work_days', $workPattern->normalizeWeeklyWorkDays()), true) ? 'checked' : '' }}>
                                    <label class="form-check-label" for="weekly_day_{{ $value }}">{{ $label }}</label>
                                </div>
                            @endforeach
                        </div>
                        <small class="text-muted d-block mt-2">{{ __('Contoh 5:2 mingguan = Senin sampai Jumat dicentang, Sabtu dan Minggu tidak dicentang.') }}</small>
                        @error('weekly_work_days')<div class="invalid-feedback d-block">{{ $message }}</div>@enderror
                        @error('weekly_work_days.*')<div class="invalid-feedback d-block">{{ $message }}</div>@enderror
                    </div>
                    <div class="col-md-3">
                        <label class="form-label">{{ __('Durasi Kerja') }}</label>
                        <input type="number" min="1" name="work_duration_value" id="workDurationValue" class="form-control @error('work_duration_value') is-invalid @enderror" value="{{ old('work_duration_value', $workPattern->work_duration_value) }}">
                        @error('work_duration_value')<div class="invalid-feedback">{{ $message }}</div>@enderror
                    </div>
                    <div class="col-md-3">
                        <label class="form-label">{{ __('Satuan Kerja') }}</label>
                    ...
                    <div class="col-md-3">
                        <label class="form-label">{{ __('Durasi Off') }}</label>
                        <input type="number" min="0" name="off_duration_value" id="offDurationValue" class="form-control @error('off_duration_value') is-invalid @enderror" value="{{ old('off_duration_value', $workPattern->off_duration_value) }}">
                        @error('off_duration_value')<div class="invalid-feedback">{{ $message }}</div>@enderror
                    </div>
                    <div class="col-md-3">
                        <label class="form-label">{{ __('Satuan Off') }}</label>
                    ...
                    <div class="col-md-6">
                        <label class="form-label">{{ __('Jam Masuk') }}</label>
                        ...
                        <small class="text-muted">{{ __('Digunakan sebagai awal rentang jam kerja.') }}</small>
                        @error('start_time')<div class="invalid-feedback">{{ $message }}</div>@enderror
                    </div>
                    <div class="col-md-6">
                        <label class="form-label">{{ __('Jam Pulang') }}</label>
                        ...
                        <small class="text-muted">{{ __('Dipakai sebagai akhir rentang kerja kotor sebelum dikurangi jadwal istirahat.') }}</small>
                        @error('end_time')<div class="invalid-feedback">{{ $message }}</div>@enderror
                    </div>
                    <div class="col-md-6">
                        <label class="form-label">{{ __('Jam Istirahat') }}</label>
                        ...
                        <small class="text-muted">{{ __('Opsional. Isi jika pola kerja punya jadwal istirahat baku.') }}</small>
                        @error('break_start_time')<div class="invalid-feedback">{{ $message }}</div>@enderror
                    </div>
                    <div class="col-md-6">
                        <label class="form-label">{{ __('Jam Kembali Bekerja') }}</label>
                        ...
                        <small class="text-muted">{{ __('Opsional. Jika diisi, sistem akan menghitung jam kerja efektif setelah istirahat.') }}</small>
                        @error('break_end_time')<div class="invalid-feedback">{{ $message }}</div>@enderror
                    </div>
                    <div class="col-12 js-sixth-day-section">
                        <div class="border rounded p-3 bg-light-subtle">
                            <h6 class="mb-1">{{ __('Jam Kerja Hari ke-6 Mingguan') }}</h6>
                            <small class="text-muted d-block mb-3">{{ __('Opsional. Dipakai khusus untuk pola mingguan yang memiliki minimal 6 hari kerja, misalnya Sabtu lebih singkat pada pola 6:1 mingguan.') }}</small>
                            <div class="row g-3">
                                <div class="col-md-6">
                                    <label class="form-label">{{ __('Jam Masuk Hari ke-6') }}</label>
                                    <input type="time" id="sixthDayStartTime" name="sixth_day_start_time" class="form-control js-sixth-day-input @error('sixth_day_start_time') is-invalid @enderror" value="{{ old('sixth_day_start_time', $workPattern->sixth_day_start_time ? \Carbon\Carbon::createFromFormat('H:i:s', $workPattern->sixth_day_start_time)->format('H:i') : '') }}">
                                    @error('sixth_day_start_time')<div class="invalid-feedback">{{ $message }}</div>@enderror
                                </div>
                                <div class="col-md-6">
                                    <label class="form-label">{{ __('Jam Pulang Hari ke-6') }}</label>
                                    <input type="time" id="sixthDayEndTime" name="sixth_day_end_time" class="form-control js-sixth-day-input @error('sixth_day_end_time') is-invalid @enderror" value="{{ old('sixth_day_end_time', $workPattern->sixth_day_end_time ? \Carbon\Carbon::createFromFormat('H:i:s', $workPattern->sixth_day_end_time)->format('H:i') : '') }}">
                                    @error('sixth_day_end_time')<div class="invalid-feedback">{{ $message }}</div>@enderror
                                </div>
                                <div class="col-md-6">
                                    <label class="form-label">{{ __('Jam Istirahat Hari ke-6') }}</label>
                                    <input type="time" id="sixthDayBreakStartTime" name="sixth_day_break_start_time" class="form-control js-sixth-day-input @error('sixth_day_break_start_time') is-invalid @enderror" value="{{ old('sixth_day_break_start_time', $workPattern->sixth_day_break_start_time ? \Carbon\Carbon::createFromFormat('H:i:s', $workPattern->sixth_day_break_start_time)->format('H:i') : '') }}">
                                    @error('sixth_day_break_start_time')<div class="invalid-feedback">{{ $message }}</div>@enderror
                                </div>
                                <div class="col-md-6">
                                    <label class="form-label">{{ __('Jam Kembali Hari ke-6') }}</label>
                                    <input type="time" id="sixthDayBreakEndTime" name="sixth_day_break_end_time" class="form-control js-sixth-day-input @error('sixth_day_break_end_time') is-invalid @enderror" value="{{ old('sixth_day_break_end_time', $workPattern->sixth_day_break_end_time ? \Carbon\Carbon::createFromFormat('H:i:s', $workPattern->sixth_day_break_end_time)->format('H:i') : '') }}">
                    ...
                    <div class="col-md-12">
                        <div class="form-check">
                            ...
                            <input class="form-check-input" type="checkbox" value="1" id="national_holiday_as_off" name="national_holiday_as_off" {{ old('national_holiday_as_off', $workPattern->national_holiday_as_off ?? true) ? 'checked' : '' }}>
                            <label class="form-check-label" for="national_holiday_as_off">
                                {{ __('Tanggal merah nasional otomatis dianggap off') }}
                            </label>
                        </div>
                        <small class="text-muted d-block mt-1">{{ __('Nonaktifkan opsi ini untuk pola seperti 10:2 atau 8:2 jika tanggal merah tetap dianggap hari kerja biasa.') }}</small>
                    </div>
                    <div class="col-md-12">
                        <div class="form-check">
                    ...
                    <div class="col-12 d-flex gap-2">
                        ...
                        <a href="{{ route('work-patterns.index') }}" class="btn btn-outline-secondary">{{ __('Batal') }}</a>
                    </div>
                </form>
            </div>
        </div>

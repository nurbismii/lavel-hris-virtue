    ...
    <div class="modal-dialog modal-lg modal-dialog-centered modal-dialog-scrollable">
        <div class="modal-content cv-update-modal">
            <div class="modal-header">
                <div>
                    <h1 class="modal-title fs-5" id="cvUpdatePreviewModalLabel">{{ __('Update HRIS dari CV Maker') }}</h1>
            ...
            <div class="modal-body">
                <div id="cvUpdatePreviewLoading" class="cv-update-state">
                    ...
                    {{ __('Memeriksa perubahan dari CV Maker...') }}
                </div>
                ...
                <div id="cvUpdatePreviewContent" class="d-none">
                    <div class="cv-update-warning mb-3">
                        ...
                        {{ __('Pilih hanya data yang telah diverifikasi. Field identitas, keuangan, organisasi, dan wilayah tidak dipilih otomatis. Bagian riwayat mengganti data yang sebelumnya bersumber dari Vitae; data manual/sumber lain tetap dipertahankan. Sinkronisasi struktur organisasi merupakan tindakan terpisah karena dapat membuat master jabatan, posisi, dan penempatan karyawan.') }}
                    </div>
                    <div class="d-flex flex-wrap gap-2 align-items-center mb-2">
                        <button type="button" class="btn btn-sm btn-outline-primary" id="cvUpdateSelectSafeButton">
                            <i class="fas fa-check-double"></i> {{ __('Pilih Field Aman') }}
                        </button>
                        <button type="button" class="btn btn-sm btn-light border" id="cvUpdateClearSelectionButton">{{ __('Kosongkan Pilihan') }}</button>
                        <span class="small text-muted" id="cvUpdateSelectionSummary">{{ __('0 item dipilih') }}</span>
                    </div>
                    <div class="table-responsive">
                        <table class="table table-sm table-bordered align-middle cv-update-table mb-0">
                            <thead>
                                <tr>
                                    <th class="text-center" style="width: 44px">{{ __('Pilih') }}</th>
                                    <th>{{ __('Field') }}</th>
                                    <th>{{ __('HRIS sekarang') }}</th>
                                    <th>CV Maker</th>
                                </tr>
                            </thead>

...
<div class="container-fluid">
    <div
        ...
        <div class="d-flex align-items-left align-items-md-center flex-column flex-md-row pt-2 pb-3">
            <div>
                <h4 class="fw-bold mb-1">
                    ...
                    {{ __('Data Presensi Keseluruhan') }}
                </h4>

                <small id="cutoffLabel" class="text-muted d-block">
                    {{ __('Pilih cutoff untuk melihat periode presensi.') }}
                </small>
                <small class="text-muted d-block">
                    {{ __('Tampilan matriks dibuat selaras dengan Setting Hari Off agar jam, status cuti, izin, off, dan alpa mudah dipindai.') }}
                </small>
            </div>
        ...
        <form class="row g-2 mb-3 align-items-end attendance-filter presensi-filter">
            ...
            <div class="col-md-3">
                ...
                <select id="filter_departemen" class="form-select form-control" disabled>
                    <option value="">{{ __('Pilih Perusahaan Dahulu') }}</option>
                    @php
                        $groupedDepartments = [];
            ...
            <div class="col-md-2">
                <label class="form-label">{{ __('Cutoff') }}</label>
        ...
        <div class="attendance-legend presensi-legend">
            <span><strong>M/I/K/P</strong> {{ __('= Masuk, Istirahat, Kembali, Pulang') }}</span>
            ...
            <span><span class="presensi-status-pill presensi-status-pill--ct">CT</span> {{ __('Cuti Tahunan') }}</span>
            <span><span class="presensi-status-pill presensi-status-pill--cr">CR</span> {{ __('Cuti Roster') }}</span>
            <span><span class="presensi-status-pill presensi-status-pill--ip">I/P</span> {{ __('Izin Berbayar') }}</span>
            <span><span class="presensi-status-pill presensi-status-pill--iu">I/U</span> {{ __('Izin Tidak Berbayar') }}</span>
            <span><span class="presensi-verification-chip is-verified">SV</span> {{ __('Verifikasi server') }}</span>
            <span><span class="presensi-verification-chip is-review">RV</span> {{ __('Review per jam') }}</span>
            <span><span class="presensi-verification-chip is-rejected">RJ</span> {{ __('Ditolak') }}</span>
        </div>

        <div class="alert alert-light border small presensi-empty-hint" id="presensiHint">
            {{ __('Pilih perusahaan, departemen, dan cutoff terlebih dahulu. Setelah itu data presensi akan dimuat dengan server-side DataTables agar tetap ringan untuk data besar.') }}
        </div>

...
@section('title', __('Master Tanggal Merah'))
    ...
    <div class="page-inner ui-page">
        <div class="ui-page-header">
            <div class="ui-page-heading">
                ...
                <div>
                    <h4 class="ui-page-title">{{ __('Master Tanggal Merah') }}</h4>
                    <p class="ui-page-subtitle">{{ __('Kelola tanggal merah nasional yang dipakai oleh jadwal kerja, presensi, dan pengaturan shift.') }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="alert ui-alert ui-alert--warning">
            {{ __('Fitur ini belum aktif karena tabel tanggal merah nasional belum tersedia. Jalankan') }} <code>php artisan migrate</code> {{ __('terlebih dahulu.') }}
        </div>
        @else
        <div class="ui-grid ui-grid--sidebar">
            <section class="ui-panel" aria-labelledby="nationalHolidayFormTitle">
                <div class="ui-panel__header">
                    <div>
                        <h5 class="ui-panel__title" id="nationalHolidayFormTitle">{{ __('Input Tanggal Merah') }}</h5>
                        <p class="ui-panel__meta">{{ __('Jika tanggal yang sama diinput ulang, nama liburnya akan diperbarui.') }}</p>
                    </div>
                </div>
                ...
                <div class="ui-panel__body">
                    <form action="{{ route('national-holidays.store') }}" method="POST" class="ui-form-grid" data-loading-text="Menyimpan...">
                        @csrf
                        <div class="ui-field">
                            <label class="form-label" for="holiday_date">{{ __('Tanggal') }}</label>
                        ...
                        <div class="ui-field">
                            <label class="form-label" for="holiday_name">{{ __('Nama Tanggal Merah') }}</label>
                            <input type="text" id="holiday_name" name="holiday_name" class="form-control @error('holiday_name') is-invalid @enderror" value="{{ old('holiday_name') }}" maxlength="150" placeholder="{{ __('Contoh: Hari Raya Idul Fitri') }}" required>
                        ...
                        <button type="submit" class="btn btn-primary ui-btn-icon w-100" data-loading-text="Menyimpan...">
                            ...
                            <span>{{ __('Simpan Tanggal Merah') }}</span>
                        </button>
                    </form>
                </div>
            </section>
            ...
            <section class="ui-panel" aria-labelledby="nationalHolidayTableTitle">
                <div class="ui-panel__header">
                    <div>
                        <h5 class="ui-panel__title" id="nationalHolidayTableTitle">{{ __('Daftar Tanggal Merah') }}</h5>
                        <p class="ui-panel__meta">Menampilkan data tahun {{ $year }}.</p>
                    </div>
                    <form method="GET" class="ui-filter-bar" data-loading-text="Memuat...">
                        <div class="ui-field">
                            <label class="form-label" for="year">{{ __('Tahun') }}</label>
                                ...
                                <tr>
                                    <td colspan="4">
                                        <div class="ui-empty-state">
                                            ...
                                            <span>{{ __('Belum ada tanggal merah nasional pada tahun ini.') }}</span>
                                        </div>
                                    </td>
                                </tr>
                                @endforelse

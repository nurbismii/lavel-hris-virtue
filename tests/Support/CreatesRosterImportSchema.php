...
trait CreatesRosterImportSchema
{
    ...
    protected function createRosterImportSchema(): void
    {
        ...
        Schema::create('users', function (Blueprint $table) { $table->string('id')->primary(); $table->string('name'); $table->string('email')->nullable(); $table->string('password')->nullable(); $table->timestamps(); });
        Schema::create('import_histories', function (Blueprint $table) { $table->id(); $table->string('import_id', 36)->nullable(); $table->string('import_type'); $table->string('status'); $table->string('file_path')->nullable(); $table->string('failure_file_path')->nullable(); $table->string('file_checksum')->nullable(); $table->longText('summary')->nullable(); $table->longText('failure_samples')->nullable(); $table->string('confirmed_by')->nullable(); $table->timestamp('expires_at')->nullable(); $table->timestamp('confirmed_at')->nullable(); $table->timestamps(); });

...
final class RosterScheduleImportValidationService
{
    public function validate(RosterWorkbookData $data): array
    {
        ...
        $pairGroups=collect($periodRows)->filter(fn($x)=>$x[1]['off_start']&&preg_match('/^\d+$/',(string)$x[0]['nik']))->groupBy(fn($x)=>$x[1]['off_start']);
        $schedules=RosterSchedule::query()->where(function($q)use($pairGroups){foreach($pairGroups as $date=>$group){$q->orWhere(function($p)use($date,$group){$p->where('off_start',$date)->whereIn('employee_nik',collect($group)->map(fn($x)=>$x[0]['nik'])->unique());});}})->get(['employee_nik','off_start','source','period_year','period_number'])->keyBy(fn($s)=>$s->employee_nik.'|'.$s->getRawOriginal('off_start'));
        ...
        foreach($periodRows as [$row,$period]) { $item=['row_number'=>$row['row_number'],'nik'=>(string)$row['nik'],'no_ktp'=>(string)$row['no_ktp'],'employee_name'=>$row['employee_name'],'hris_name'=>null,'year'=>$period['year'],'period_number'=>$period['period_number'],'source_column'=>$period['source_column'],'off_start'=>$period['off_start'],'action'=>'create','errors'=>[],'warnings'=>[]];
            ...
            if(!preg_match('/^\d{16}$/',$ktp)||($row['identity_error']??null)==='unsafe_numeric_identity')$add('invalid_ktp','Nomor KTP harus 16 digit teks');
            $employee=$employees->get($nik); if($nik!==''&&!$employee)$add('employee_not_found','Karyawan tidak ditemukan'); if($employee){$item['hris_name']=$employee->nama_karyawan; $stored=(string)$employee->no_ktp; if(!preg_match('/^\d{16}$/',$stored))$add('invalid_ktp','KTP HRIS tidak valid'); elseif(preg_match('/^\d{16}$/',$ktp)&&!hash_equals($stored,$ktp))$add('ktp_mismatch','Nomor KTP tidak sesuai'); if($row['employee_name']!==''&&$row['employee_name']!==$employee->nama_karyawan)$add('name_mismatch','Nama berbeda',true); if(strtoupper((string)$employee->status_resign)!=='AKTIF')$add('inactive_employee','Karyawan tidak aktif',true);}
            if(($period['cell_error']??null)||!$period['off_start'])$add('invalid_date','Tanggal roster tidak valid'); $pair=$nik.'|'.$period['off_start']; if($period['off_start']&&isset($seen[$pair]))$add('duplicate_off_start','Tanggal off duplikat');$seen[$pair]=true; $existing=$schedules->get($pair); if($existing&&$existing->source==='manual')$add('manual_conflict','Jadwal manual tidak boleh ditimpa'); elseif($existing)$item['action']=((int)$existing->period_year===(int)$period['year']&&(int)$existing->period_number===(int)$period['period_number'])?'unchanged':'update'; if($item['errors'])$item['action']='blocked'; if($period['raw_remark']&&stripos($period['raw_remark'],'review')!==false)$add('remark_need_review','Remark perlu review',true); $rows[]=$item;
        }
